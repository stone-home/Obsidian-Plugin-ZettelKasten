...
export class StepByStepFolderModal extends Modal {
	...
	private display() {
		...
		headerEl.createEl('h3', { text: `Current: ${this.currentFolder.path}` });

		// --- 2. Content: Folder List ---
		const children = this.currentFolder.children.filter(child => child instanceof TFolder) as TFolder[];
		const folderListEl = contentEl.createDiv('folder-list');

		if (children.length === 0) {
			folderListEl.createEl('div', { text: 'No sub-folders.', cls: 'empty-list-notice' });
		} else {
