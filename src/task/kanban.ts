...
export class WeeklyKanban {
	...
	private createKanbanNote(): BaseDefault {
		this.logger.info(
			`Creating weekly kanban for week ${this.getCureentWeekNumber()}`,
		);

		const kanban = this.factory.createNote(
			NoteType.FLEETING,
		) as BaseDefault;
		...
		kanban.setProperty(
			"year",
			format(new Date(), this.settings.dateFormat),
		);
		return kanban;
	}
	...
	public async kanbanCreate(open: boolean = true) {
		...
		if (Utils.fileExists(this.app, previousWeekKanban, false)) {
			this.logger.info(
				`Loading previous week kanban note: ${previousWeekKanban}`,
			);
			const previousNote =
				await this.factory.loadFromFile(previousWeekKanban);
			const backlogSection = previousNote
				.getBody()
				.getSection("Backlogs", 2);
			const inProgressSection = previousNote
				.getBody()
				.getSection("In Progress", 2);
			if (backlogSection) {
	...
	public async weeklyTaskSummary(): Promise<void> {
		this.logger.info(
			`Creating weekly summary note for week ${this.getCureentWeekNumber()}`,
		);
		const summary = this.factory.createNote(
			NoteType.FLEETING,
		) as BaseDefault;
		summary.setTitle(this.getSummaryNoteName());
		summary.setPath(this.getKanbanDir());
		summary.addBodyContent(
			`- kanban link: \[\[${this.getKanbanNoteName()}|here\]\]`,
			"**🔗Source**",
			4,
		);
	...
	public async taskCreate(title: string) {
		...
		this.logger.info(
			`Creating weekly task for week ${this.getCureentWeekNumber()}`,
		);
